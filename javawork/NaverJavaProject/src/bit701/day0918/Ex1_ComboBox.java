...
public class Ex1_ComboBox extends JFrame{
	...
	public Ex1_ComboBox(String title) {
		...
		this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//프레임 종료 해주는  메서드
		this.setDesign();
		...
		lblMessage.setFont(new Font("Comics Sans MS", Font.BOLD, 30));
		p.add(lblMessage);
		...
		//라디오 버튼들을 같은 그룹으로 묶기 (그래야 4개 중 한 개만 선택한다)
		bg.add(rbYellow);
		bg.add(rbBlue);
		bg.add(rbMagenta);
		bg.add(rbGray);
		
		//4개의 라디오 버튼을 일단 패널에 묶어서 너흔 후 프레임에 추가하기
