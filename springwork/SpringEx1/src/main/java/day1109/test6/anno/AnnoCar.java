...
@Component("mycar") //id 가 mycar 이 된다
//(명시하지않으면 Class Name 이 된다)
public class AnnoCar {
	
	//@Autowired //자동 주입 - 에러 발생 : 매칭 가능한 클래스가 2개 이상이므로 오류가 발생
	//AnnoTire annoTire;
	
	//인터페이스를 구현한 클래스가 여러 개인 경우 정확하게 클래스명으로 선언하면 된다
	//유일하기 때문에 알아서 자동으로 주입된다
	//@Autowired
	//AnnoCanadaTire annoTire;
	//AnnoKoreaTire annoTire;
	
	//Resource 는 인터페이스를 구현한 클래스가 여러 개일 경우 콕 집어서 정해준다 (누구를 주입할지)
