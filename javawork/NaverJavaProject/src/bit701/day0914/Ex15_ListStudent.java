...
public class Ex15_ListStudent {
	...
	{
		int num=0;
		System.out.println("\n1.학생 추가  2.학생 삭제  3.학생 검색  4.전체 출력  5.종료");
		System.out.print("선택하세요>>");
		try {
			num=Integer.parseInt(sc.nextLine());
		}catch(NumberFormatException e) {
			num=4;//번호에 문자가 들어간 경우 4번 전체 출력으로 하기
		}
		return num;
	}
	...
	public void searchStudent()
	{
		...
		//이름을 입력 후 ㅐ당 학생이 있는 경우 이름, 점수, 혈액형 출력
		...
		for(int i=0;i<list.size();i++)
		{
			...
			{
				find=true;
				System.out.println("이름:"+s.getName()+", 점수:"+s.getScore()+", 혈액형:"
						+s.getBlood().toUpperCase()+"형");
				break;
			}
		}
		if(!find)
	...
	{
		System.out.println("학생 정보 정보 출력");
