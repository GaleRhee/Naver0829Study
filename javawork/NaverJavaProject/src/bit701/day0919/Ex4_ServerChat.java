...
public class Ex4_ServerChat extends JFrame implements Runnable{
	...
	public void run() {
		...
		try {
			...
			{
				//대기 중이다가
				//접속 요청하는 클라이언트 허용, 허용이 되면 Socket이 만들어진다
				...
				this.autoScroll();//마지막 줄로 위치 자동 변경 ** 중요**
			}
		} catch (IOException e) {
	...
	class ClientMember extends Thread
	{
		...
		public void run() {
			...
			//언제 클라이언트들이 메세지를 보낼지 모르므로 while문을 열어 놓는다
			while(true)
			{
				//클라이언트들이 보낸 메세지를 읽는다
				try {
					...
					// TODO Auto-generated catch block//너무 느려지니 주석 처리
					// e.printStackTrace();
				}
			}
	...
	private void autoScroll()
	{
		...
		area.setCaretPosition(n);//마지막 줄에 위치 변경
	}
