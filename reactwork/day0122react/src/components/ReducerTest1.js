...
//Reducer 를 이용해서 입출금 관리를 하는 간단한 예제입니다
//action.type 을 미리 상수화해서 정해 놓고자 할 경우 
const ACTION_TYPES = {
    add: 'addmoney',
    sub: 'submoney'
}

//ACTION_TYPES.add 로 지정 가능, 입출금 미리 세팅을 하고 밑에서 호출
...
const ReducerTest1 = () => {
    ...
    const [money, dispatch] = useReducer(reducer, 0); //이벤트를 처리할 기관 (reducer), 0 은 money 초기값 설정
    ...
    return (
        <div>
            ...
            <input type='text' placeholder='입금 또는 출금액 입력'
