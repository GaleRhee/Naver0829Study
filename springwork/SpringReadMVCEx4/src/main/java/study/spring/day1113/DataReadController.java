...
public class DataReadController {
	...
	//@RequestParam 자체가 사실 생략 가능하다 (하지만 다른 어노테이션으로 잘못 인식하는 경우를 방지하기 위해 생략하지 않는 게 좋기는 함)
...
/*	public String process1(HttpServletRequest request, Model model)
	{
		//jsp 처럼 읽어 보자 (스프링에서는 이렇게 읽지는 않음)
	...
	public String process2(Model model,@RequestParam String message,@RequestParam String animal,
			...
			//form tag 에 name 이라는 태그는 없음
			//null 값이 들어오더라도 에러가 안나게 할 경우 (에러 코드 400) required = false
			//null 일 경우 기본값을 "다니엘"로 하겠다는 뜻
	{
	...
	//폼 태그의 name 이 key 로, 입력값이 value 로 저장
	{
