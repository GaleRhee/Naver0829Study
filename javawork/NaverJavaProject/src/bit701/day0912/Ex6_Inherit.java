...
class SubA extends SuperA
{
	public void show()
	{
		//this는 자기자신의 주소를 가진 인스턴스 변수이다
		//this로 processA() 호출 시 현재 클래스에 없으면 부모로부터 찾아야 가져온다
		this.processA();
		this.processB();
		//부모의 주소값을 가진 인스턴스 변수 super
		//반드시 super로만 부모 메서드를 가져와야 하는 경우는
		//부모의 메서드를 자식이 오버라이드한 경우, this와 다른 결과를 가져옴
		super.processA();//부모의 메서드이므로 super로로 가져올 수 있다
		super.processB();
	}
}
...
public class Ex6_Inherit {
	...
	public static void main(String[] args) {
		...
		sub1.processA();
		sub1.processB();
		//sub1.processC();//상속 안됨 (private은 패키지 상관 없이 상속 안됨, 오류 발생
		System.out.println("--------");
		sub1.show();
	}
