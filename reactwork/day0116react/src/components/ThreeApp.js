...
function ThreeApp(props) {
    ...
    //입력 시 inputs 의 해당 멤버 변수로 값이 들어가게 하기 위한 이벤트
    const inputChangeEvent = (e) => {
        const { name, value } = e.target; //이벤트가 발생하는 태그의 name 값과 입력값 얻기
        setInputs({
            ...inputs, //기존의 값을 펼침 연산자로 일단 넣고
            [name]: value //입력한 태그의 name 에 값 변경
        })
    }

    //배열 추가 이벤트
    const addArrayEvent = () => {
        //기본 배열 데이터에 묶음 변수인 inputs 추가
    ...
    //삭제 이벤트에서 호출해야 함 
    //(중간에 e : event 이외에는 들어가면 안 되며 들어가면 일반 함수)
    ...
    return (
        <div style={{ width: '450px' }}>
            <Alert severity='success'>
                <h3>ThreeApp - 하나의 변수에 여러 값 지정하기</h3>
            </Alert>
