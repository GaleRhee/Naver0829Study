...
<script type="text/javascript">
	...
	$(function(){
		...
		$(document).on("click",".memberdel",function(){
			let a=confirm("해당 회원을 탈퇴 시키려면 [확인]을 눌러주세요");
			if(a){
				...
				$.ajax({
					...
					success:function(res){
						word="";//전역 변수라 마지막 검색 단어를 갖고 있으므로 초기화 해준다
			...
			success:function(res){
				...
				$.each(res,function(idx,item){
					...
					s+=
						...
						<tr>
							<td>
								<img src="\${imgsrc}" class="rounded-circle" width=30 height=30
								   hspace=5 border=1>
								\${item.name}
							</td>
							<td>\${item.myid}</td>
...
<c:if test="${sessionScope.loginok==null}">
		<h4 style="margin: 50px;color: red;">회원 명단을 보려면 먼저 로그인을 해주세요</h4>
</c:if>
<c:if test="${sessionScope.loginok!=null}">
	<div>
